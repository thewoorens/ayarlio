...
import {checkLimit, incrementUsage} from "@/lib/plan/checkLimit";
...
export async function POST(req: NextRequest) {
    try {
        ...
        await checkLimit(tenantId, "maxStaff", 1);
        ...
        await incrementUsage(tenantId, "maxStaff", 1);
        ...
        if (error.message === "LIMIT_EXCEEDED") {
            return NextResponse.json(
                {
                    success: false,
                    message: "Personel limitine ulaştınız. Plan yükseltin.",
                },
                {status: 403}
            );
        }
